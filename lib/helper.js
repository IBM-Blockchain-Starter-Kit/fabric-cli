...
function FabricHelper(networkConfigFilePath, channelName, keyValueStoreBasePath) 
{
	console.log(networkConfigFilePath);
	hfc.addConfigFile(networkConfigFilePath);
...
FabricHelper.prototype.setupPeers = function (channel, org, client) {
	for (let key in this.ORGS[org].peers) {
		let data = fs.readFileSync(path.join(__dirname, this.ORGS[org].peers[key]['tls_cacerts']));
		let peer = client.newPeer(
			this.ORGS[org].peers[key].requests,
			{
				pem: Buffer.from(data).toString(),
				'ssl-target-name-override': this.ORGS[org].peers[key]['server-hostname']
			}
		);
		peer.setName(key);
...
FabricHelper.prototype.newOrderer = function (client) {
	...
	return client.newOrderer(this.ORGS.orderer.url, {
		'pem': caroots,
		'ssl-target-name-override': this.ORGS.orderer['server-hostname']
	});
}
...
FabricHelper.prototype.newRemotes = function (names, forPeers, userOrg) {
	...
	for (let idx in names) {
		...
		if (this.ORGS[userOrg].peers[peerName]) {
			...
			let data = fs.readFileSync(path.join(__dirname, this.ORGS[userOrg].peers[peerName]['tls_cacerts']));
			let grpcOpts = {
				pem: Buffer.from(data).toString(),
				'ssl-target-name-override': this.ORGS[userOrg].peers[peerName]['server-hostname']
			};
...
FabricHelper.prototype.processChaincodeInstantiateProposal = function(transactionId, proposalResult, org) {
	...
	let data = fs.readFileSync(path.join(__dirname, this.ORGS[org].peers['peer1'][
		'tls_cacerts'
	]));
