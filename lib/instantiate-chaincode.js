...
var instantiateChaincode = function (connectionProfilePath, channelName, chaincodeName, chaincodeVersion, args, 
	functionName, org, timeout, endorsementPolicy, cryptoDir) {
	logger.debug('\n============ Instantiate chaincode on organization ' + org +
		' ============\n');

	let helper = new FabricHelper(connectionProfilePath, channelName, path.join(process.env.HOME, 'fabric-client-kvs'), cryptoDir);
	...
	return helper.getOrgAdmin(org).then((user) => {
		...
		if (endorsementPolicy) {
			...
			request.endorsementPolicy = JSON.parse(endorsementPolicy);
		...
		return channel.sendInstantiateProposal(request, timeout).then(
			(results) => {
